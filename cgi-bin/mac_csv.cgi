...
awk -F';' '{
    ...
    printf "    edit \"%s\"\n", $1
    printf "        set type mac\n"
    printf "        set macaddr %s\n", $2
    for (i=4; i<=NF; i++) {
        printf "        set field%d %s\n", i, $i
    }
    printf "    next\n"
    printf "end\n"
}' "$TEMP_FILE" > "$UPLOAD_DIR/mac_file.txt"

# Ajusta as permissões do arquivo
chmod 644 "$UPLOAD_DIR/mac_file.txt"

# Remove o arquivo temporário
rm "$TEMP_FILE"

# Define o conteúdo a ser exibido no <textarea>
TEXTAREA_CONTENT=$(cat "$UPLOAD_DIR/mac_file.txt")
...
echo "<p><a href='/uploads/mac_file.txt' download='mac_file.txt'>Baixar arquivo</a></p>"
