...
#COOKIE=$(echo "$HTTP_COOKIE" | grep -E -o 'session=authenticated')
#
#if [ "$COOKIE" != "session=authenticated" ]; then
#    # Sessão inválida, redirecionar para login
#    echo "Location: /login.html"
#    echo "Content-type: text/html"
#    echo "Set-Cookie: session=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT" # Limpa o cookie
#    echo ""
#    exit 0
#fi
...
awk -F';' '{
    ...
    printf "    next\n"
    printf "end\n"
}' "$TEMP_FILE" > "$UPLOAD_DIR/address_file.txt"

# Ajusta as permissões do arquivo
chmod 644 "$UPLOAD_DIR/address_file.txt"

# Remove o arquivo temporário
rm "$TEMP_FILE"

# Define o conteúdo a ser exibido no <textarea>
TEXTAREA_CONTENT=$(cat "$UPLOAD_DIR/address_file.txt")
