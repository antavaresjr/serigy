...
awk -F';' '{
    ...
    printf "    edit \"%s\"\n", $1
    printf "        set extip \"%s\"\n", $2
    printf "        set mappedip \"%s\"\n", $3
    printf "        set extintf \"any\"\n"
    printf "        set portforward enable\n"
    printf "        set status disable\n"
    printf "        set protocol %s\n",$4
    printf "        set extport \"%s\"\n", $5
    printf "        set mappedport \"%s\"\n", $6
    for (i=8; i<=NF; i++) {
        printf "        set field%d %s\n", i, $i
    }
    printf "    next\n"
    printf "end\n"
}' "$TEMP_FILE" > "$UPLOAD_DIR/vip_file.txt"

# Ajusta as permissões do arquivo
chmod 644 "$UPLOAD_DIR/vip_file.txt"

# Remove o arquivo temporário
rm "$TEMP_FILE"

# Define o conteúdo a ser exibido no <textarea>
TEXTAREA_CONTENT=$(cat "$UPLOAD_DIR/vip_file.txt")
...
echo "<p><a href='/uploads/vip_file.txt' download='vip_file.txt'>Baixar arquivo</a></p>"
